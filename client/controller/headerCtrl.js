...
angular.module('routerApp').controller('headerCtrl',
  ...
  function ($http,$scope,$rootScope, $location,$timeout,$filter) {
	...
	******************************/
	...
	var refresh=function(){
			...
			$scope.monthlyExpense=0;
			$scope.weeklyExpense=0;
		$http.get('/user/expenses_monthly').success(function(response){
			for(var i=0;i<response.length;i++){
				if(response[i].day==parseInt($scope.time.today)){
					$scope.dailyExpense+=response[i].amount;
				}
				if(response[i].day>=firstday && response[i].day<=lastday){
					$scope.weeklyExpense+=response[i].amount;
				}
				$scope.monthlyExpense+=response[i].amount;
